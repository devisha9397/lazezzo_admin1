...
$first_page=1;
							...
							$obj=new database();
							$res=$obj->getAllOrderByDisApproved1($restid,$page1,$noi);
				...
				//	echo '<td><font size="4" color="DarkRed">'.$row["fk_rest_id"].'</font>';
					...
					echo '<td><a href="approve.php?id='.$row["order_id"].'"><button style="background-color: green" type="button" class="btn btn-default" aria-label="Left Align"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span></button></a>';
					echo '<td><a href="orderdel.php?id='.$row["order_id"].'"><button style="background-color: red" type="button" class="btn btn-default" aria-label="Left Align"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button></a>';
					echo '</tr>';
			
						}
				$res1=$obj->getAllOrderByDisApproved($restid);
...
<?php
			...
			{	
			echo '<a href="disapproved.php?page='.$first_page.'" style="text-decoration:none;"><button class="button"><<</button></a>';	
			}
			if($prev_page==0)
			{
			...
			{
		echo '<a href="disapproved.php?page='.$prev_page.'" style="text-decoration:none;"><button class="button">Previous</button></a>';	
			}
			
			for($b=1;$b<=$a;$b++)
		{
			echo '<a href="disapproved.php?page='.$b.'" style="text-decoration:none;"><button class="button">'.$b.'</button></a>'; 
		}
		
		if($next_page==$a)
		{
			echo '<a href="disapproved.php?page='.$next_page.'" style="text-decoration:none;"><button class="button">Next</button></a>';	
		}
		else
		{	
